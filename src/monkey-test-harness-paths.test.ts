...
import { readHarnessFile, writeMonkeyHarness } from "./monkey-test-harness.js";
...
it("reads ordinary scaffold without O_NOFOLLOW and rejects a swapped symlink", async () => {
  await withHarness(async (root) => {
    const spec = join(root, "tests/monkey/monkey.spec.ts");
    const original = await fs.lstat(spec);
    assert.match(
      (await readHarnessFile(spec, original, null)) ?? "",
      /Generated by kit monkey-test/,
    );
    await fs.rm(spec);
    const config = join(root, "playwright.monkey.config.ts");
    await fs.symlink(config, spec);
    assert.equal(await readHarnessFile(spec, original, null), null);
    await fs.rm(spec);
