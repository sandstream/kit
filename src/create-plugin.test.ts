...
describe("createPlugin", () => {
  ...
  it("creates package.json with correct name", async () => {
    ...
    const raw = await readFile(
      join(tmpDir, "kit-plugin-pkg-check", "package.json"),
      "utf-8"
    );
  ...
  it("creates tsconfig.json", async () => {
    ...
    const raw = await readFile(
      join(tmpDir, "kit-plugin-ts-check", "tsconfig.json"),
      "utf-8"
    );
  ...
  it("creates src/index.ts that exports { adapter }", async () => {
    ...
    const src = await readFile(
      join(tmpDir, "kit-plugin-idx-check", "src", "index.ts"),
      "utf-8"
    );
  ...
  it("creates README.md", async () => {
    ...
    const readme = await readFile(
      join(tmpDir, "kit-plugin-readme-check", "README.md"),
      "utf-8"
    );
  ...
  it("generated package.json has typescript as devDependency", async () => {
    ...
    const raw = await readFile(
      join(tmpDir, "kit-plugin-sdk-dep-check", "package.json"),
      "utf-8"
    );
