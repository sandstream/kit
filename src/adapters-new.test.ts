...
describe("planetscaleDbAdapter", () => {
  ...
  it("provision returns error when service token credentials are missing", async () => {
    ...
    assert.ok(
      result.message.includes("PLANETSCALE_SERVICE_TOKEN_ID"),
      `missing in message: ${result.message}`,
    );
    assert.ok(
      result.message.includes("PLANETSCALE_SERVICE_TOKEN"),
      `missing in message: ${result.message}`,
    );
  ...
  it("provision reports which specific credentials are missing", async () => {
    ...
    assert.equal(result.success, false);
    assert.ok(
      result.error?.includes("PLANETSCALE_SERVICE_TOKEN"),
      `expected missing token in error: ${result.error}`,
    );
    assert.ok(
      result.error?.includes("PLANETSCALE_ORG"),
      `expected missing org in error: ${result.error}`,
    );
  });
});
...
describe("loopsEmailAdapter", () => {
  ...
  it("check returns true when LOOPS_API_KEY is present", async () => {
    const result = await loopsEmailAdapter.check(mockContext({ LOOPS_API_KEY: "loops_abc123" }));
    assert.equal(result, true);
  });
  ...
  it("provision returns error with setup instructions when key is missing", async () => {
    ...
    assert.equal(result.success, false);
    assert.ok(
      result.message.includes("loops.so"),
      `expected dashboard URL in message: ${result.message}`,
    );
    assert.ok(
      result.message.includes("LOOPS_API_KEY"),
      `expected key name in message: ${result.message}`,
    );
  });
});
...
describe("liveblocksRealtimeAdapter", () => {
  ...
  it("provision returns error with setup instructions when key is missing", async () => {
    ...
    assert.equal(result.success, false);
    assert.ok(
      result.message.includes("liveblocks.io"),
      `expected dashboard URL: ${result.message}`,
    );
    assert.ok(
      result.message.includes("LIVEBLOCKS_SECRET_KEY"),
