 ...
 * is worth. Missing, non-executable, or outdated is a FAIL: at least one declared
 * command does not run. An older hook may still run other commands, but cannot
 * satisfy the current contract.
 */
export function hookCheckStatus(h: HookCheckResult): "pass" | "fail" {
  return h.installed && h.executable && h.upToDate ? "pass" : "fail";
}
...
async function checkHook(
  ...
  try {
    if (lstatSync(hookPath).isSymbolicLink()) {
      return {
        ...
        executable: false,
        upToDate: false,
        detail: "symbolic link hook refused; use a regular executable hook file",
      };
    }
  } catch {
    // The missing-file result below includes dangling links and absent paths.
  }
  ...
  return checkHookContent(hookPath, hookName, commands);
}

async function checkHookContent(
  hookPath: string,
  hookName: string,
  commands: string[],
): Promise<HookCheckResult> {
  try {
    ...
    const external = !content.includes("# Generated by kit");
    return {
      hookName,
      installed: true,
      executable: true,
