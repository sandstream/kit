...
async function installBypassDetector(hooksDir: string): Promise<HookInstallResult[]> {
  ...
  try {
    ...
    const preCommitState = await hookPathState(preCommitDetectorPath);
    if (preCommitState === "symlink") {
      throw new Error(`Refusing symbolic-link hook: ${preCommitDetectorPath}`);
    }
    if (preCommitState === "present") {
  ...
  try {
    ...
    const postCommitState = await hookPathState(postCommitDetectorPath);
    if (postCommitState === "symlink") {
      throw new Error(`Refusing symbolic-link hook: ${postCommitDetectorPath}`);
    }
    if (postCommitState === "present") {
