...
export interface HookFloorReport {
  ...
  exists: boolean;
  /** Enforced hook contracts; built-in names only when the configured command is that built-in. */
  installed: string[];
  /**
...
function configuredHooks(dir: string, config?: kitConfig): string[] {
  ...
  for (const [file, builtin] of Object.entries(BUILTIN_HOOK_FILES)) {
    ...
    const commands = configuredCommands?.length ? configuredCommands : [builtin.command];
    if (missingHookCommands(body, commands).length === 0) {
      const isBuiltinContract =
        !configuredCommands?.length ||
        configuredCommands.some((command) => command.includes(builtin.command));
      installed.push(isBuiltinContract ? builtin.name : file);
    }
  }
...
export function describeHookFloor(cwd = process.cwd(), config?: kitConfig): HookFloorReport {
  ...
  const installed = configuredHooks(dir, config);
...
export function judgeHookFloor(report: HookFloorReport): HookFloorVerdict {
  ...
  if (report.installed.length === 0) {
    return {
      ...
      detail: report.hooksPath
        ? `no enforcing git hooks in ${where}`
        : "no enforcing git hooks installed here",
    };
  }
  ...
  return {
    status: "pass",
    detail: `${report.installed.length} enforcing git hook(s) wired in ${where}: ${report.installed.join(", ")}`,
  };
}
