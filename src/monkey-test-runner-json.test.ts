...
for (const outcome of ["test command success", "test failure", "seed failure", "browser skipped"]) {
  it(`MSG-04: --json stays parseable with noisy child output (${outcome})`, async () => {
    ...
    try {
      ...
      if (outcome === "test command success" || outcome === "test failure") {
      ...
      if (outcome === "test command success")
        assert.ok(
          report.findings.some(
            (finding) => finding.title === "Custom test command cannot establish browser evidence",
          ),
        );
