...
describe("uninstallHooks — result contract and destructive edges", () => {
  ...
  it("removes only the injected sentinel block from an externally managed hook", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const original = "#!/bin/sh\necho 'my own gate'\n";
    try {
      await writeFile(join(hooksDir, "pre-commit"), original, "utf-8");
      await installHooks({}, gitDir, base);
      assert.notEqual(await readFile(join(hooksDir, "pre-commit"), "utf-8"), original);

      const [res] = await uninstallHooks(["pre-commit"], gitDir, base);

      assert.equal(res.action, "installed");
      assert.match(res.detail, /externally managed hook preserved/);
  ...
  it("never follows an existing hook symlink during install", async () => {
    ...
    try {
      ...
      const sentinel = results.find((result) => result.hookName === "pre-commit (sentinel)");
