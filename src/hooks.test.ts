 ...
 * the ownership, containment, and read-only guards around those deletions.
 */
describe("uninstallHooks — result contract and destructive edges", () => {
  ...
  it("leaves a hook kit did not generate untouched", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    try {
      await writeFile(join(hooksDir, "pre-push"), "#!/bin/sh\necho 'my own gate'\n", "utf-8");
      ...
      assert.match(res.detail, /externally managed/);
  ...
  it("preserves an externally managed hook through install and uninstall", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const original = "#!/bin/sh\necho 'my own gate'\n";
    try {
      await writeFile(join(hooksDir, "pre-commit"), original, "utf-8");
      await installHooks({}, gitDir, base);
      assert.equal(await readFile(join(hooksDir, "pre-commit"), "utf-8"), original);
  ...
  it("never follows an existing hook symlink during install", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const outside = join(base, "operator-script");
    const original = "#!/bin/sh\necho operator\n";
    try {
      await writeFile(outside, original, "utf-8");
      await symlink(outside, join(hooksDir, "pre-commit"));

      const results = await installHooks({}, gitDir, base);
      const sentinel = results.find((result) => result.hookName === "pre-commit");

      assert.equal(sentinel?.action, "failed");
      assert.match(sentinel?.detail ?? "", /symbolic-link hook/i);
      assert.equal(await readFile(outside, "utf-8"), original);
    } finally {
      await rm(base, { recursive: true, force: true });
    }
  });

  it("leaves an external pre-commit hook untouched while installing configured hooks", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const original = "#!/bin/sh\necho operator\n";
    try {
      await writeFile(join(hooksDir, "pre-commit"), original, { mode: 0o755 });
      const results = await installHooks({ "pre-commit": ["npm audit"] }, gitDir, base);
      assert.equal(await readFile(join(hooksDir, "pre-commit"), "utf-8"), original);
      assert.equal(existsSync(join(hooksDir, "post-commit")), false);
      assert.ok(
        results.some((result) => result.hookName === "pre-commit" && result.action === "skipped"),
      );
    } finally {
      await rm(base, { recursive: true, force: true });
    }
  ...
  it("leaves an external post-commit hook untouched while installing the detector", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const original = "#!/bin/sh\necho operator post-commit\n";
    try {
      await writeFile(join(hooksDir, "post-commit"), original, { mode: 0o755 });
      const results = await installHooks({}, gitDir, base);
      assert.equal(await readFile(join(hooksDir, "post-commit"), "utf-8"), original);
      assert.ok(
        results.some(
          (result) =>
            result.hookName === "post-commit (bypass-detector)" && result.action === "skipped",
        ),
      );
    } finally {
      await rm(base, { recursive: true, force: true });
  ...
  it("refuses a symlink before writing any configured hooks or detector files", async () => {
    const { base, gitDir, hooksDir } = await fixture();
    const target = join(base, "operator-script");
    try {
      await writeFile(target, "#!/bin/sh\necho operator\n", { mode: 0o755 });
      await symlink(target, join(hooksDir, "pre-commit"));
      const results = await installHooks({ "pre-push": ["npm audit"] }, gitDir, base);
      assert.ok(results.some((result) => result.action === "failed"));
      assert.equal(existsSync(join(hooksDir, "pre-push")), false);
      assert.equal(existsSync(join(hooksDir, "post-commit")), false);
    } finally {
      await rm(base, { recursive: true, force: true });
    }
