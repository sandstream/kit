...
| `[policy.agent_writes]` makes agent-permitted scopes EXPLICIT, not implicit | ✅ **enforced for the ops in the registry, in kit AND in the plugin write surfaces** — `POLICY_OPS` (`policy-gate.ts`) is the single op vocabulary both the caller and the operator read. Coverage is the six propagation targets' `env_set`, Supabase rotation (`scoped_key_mint`, `jwt_secret_roll` and `scoped_key_revoke` are SEPARATE ops because their blast radii are not comparable — the roll invalidates every live token, the revoke kills one key something is authenticating with), and the plugin ops: `resolve_issue`, `create_release`, `trigger_deploy`, `env_unset`, `api_token_revoke`, `webhook_create`, `webhook_delete`. The plugins cannot import kit-core, so what crosses that boundary is the resolved DECISION (`KIT_POLICY_DENY`), not the config — a membership test with no rule in it cannot drift from `policyDecision`, whereas seven copies of the four-state rule could. That channel is exactly as strong as `KIT_READ_ONLY` and no stronger: a process that never ran kit sees no denials, and a plugin-side refusal is not audited because a plugin has no path to the governed project's log. Enforced decisions inside kit ARE audited: `enforcePolicy()` records refusals and grants with the vendor, op, state and policy hash, and `checkPolicy()` delegates to the same `policyDecision`, so one rule has one implementation — two would be the divergence class that left `kit_fix`'s MCP handler with a stale copy of the lock step. **What this row used to claim, and why the claim was the problem:** it cited `unknownPolicyEntries()` as reporting an unrecognised op to the operator. That function had no production caller. Measured: a repo with `vercel = ["env-set"]` produced a full `kit check` with zero mention of it, exit 0 — while the typo silently INVERTED the intent, since declaring the vendor refuses every op not listed. Citing a decision function as evidence of a control is the same reasoning that let `checkPolicy` sit uncalled through an entire arc. It now reports through the `policy agent-writes` row (`check-policy-ops.ts`) wired into `checkSecurity()` — the function every surface calls, so `kit ci` and `kit heal` see it too — and `src/plugin-write-gates.test.ts` derives the gated surfaces from the plugin sources, so a new ungated write fails a test instead of shipping. |
...
| `[governance.audit].remote = true` opt-in for centralized log shipping with retry-queue | ✅ shipped — `[governance.audit].company_id` supplies the tenant to every caller; POST and pending-queue paths are exercised with HTTP stubs, and queue files follow the governed repo cwd. |
