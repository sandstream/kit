...
> Complete reference for every `kit <subcommand>`. Last updated 2026-06-25 (kit 1.33.2).
...
| `kit setup`                                        | 6-step orchestrator: install → hooks → login → secrets → agent-config → verify, preceded by a network-posture prompt (connected vs air-gapped enclave → writes `[air_gap]`). `--recommended` also wires memory + git secret-scan/context-check gates. |
...
| `kit audit verify`                                    | Verify the audit log's tamper-evident hash chain (exit 1 on break).       |
