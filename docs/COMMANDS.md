...
| `kit memory install` / `uninstall`                                          | Wire (or remove) `UserPromptSubmit`, `SessionStart`, and `SessionEnd` in Claude Code (`~/.claude/settings.json`) and Codex (`~/.codex/hooks.json`); Codex hooks require `/hooks` trust.                      |
