...
receipts. That is already the repo shape: model-shaped work lives above kit; kit
owns the deterministic floor; `sandstream-kit-plugin-aisle` converts AISLE
nano-analyzer output into `.kit-scan-results.jsonl`.
...
- AISLE's own public claim is broader: it says AISLE found 29 valid curl
  ...
  release, and that all six were reported by Joshua Rogers of AISLE Research.
  This is a first-party product claim; curl's advisories corroborate the six
  2026 finder credits, not the earlier 29/5 claim. Source:
...
- Public kernel artifacts show an AISLE-assisted VT tty-reference patch by
  Joshua Rogers, signed off by Greg Kroah-Hartman; NVD tracks that VT fix as
  CVE-2026-74675. A separate VCC port-lock patch is AISLE-assisted in
  LKML/kernel mirror artifacts, but no CVE is cited here. Sources:
...
- Current findings emitted by an external scanner, including AISLE nano-analyzer
  output after the scanner has run. kit's generic contract ingests
  `.kit-scan-results.jsonl`, and the AISLE plugin writes that shape from local
  `triage.json` / `summary.json` output. Sources:
  [docs/EXTERNAL_FINDINGS.md](../EXTERNAL_FINDINGS.md) and
  [packages/kit-plugin-aisle/src/scan.ts](../../packages/kit-plugin-aisle/src/scan.ts).
...
2. Scanner either runs through `kit scan` / `kit ingest` as SARIF/OSV, or writes
   `.kit-scan-results.jsonl` for `kit check` external-finding ingest. AISLE
   nano-analyzer can use the existing `sandstream-kit-plugin-aisle` adapter,
   which reads local `triage.json` and optional `summary.json` and appends
   validated findings.
...
The JSONL external-finding contract supports the `kit check` path; SARIF/OSV
support lives in `kit scan` / `kit ingest`. Both keep the model/research engine
outside kit core. Source: [docs/EXTERNAL_FINDINGS.md](../EXTERNAL_FINDINGS.md).
