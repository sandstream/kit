...
- Security ar brett uppsatt i CI: npm audit, Snyk, bumblebee, SAST, container/IaC,
  gitleaks, compliance och en aggregerad fail-closed security gate
...
- Security-resultaten markerar `didNotRun`: scanner saknad, token saknad, crash
  eller timeout ska inte kunna lasa som gront i strikt gate (`src/check-security.ts:83-91`).
...
- Root lint missar workspaces: `npm run lint` ar `eslint src`, medan build kor
  alla plugin-workspaces (`package.json:28-36`). Det kravde separat lint for
  `packages/kit-plugin-aisle/src`.
