...
describe("listOrganizations (network error path)", () => {
  ...
  it("redacts the bearer token in API error bodies without hiding diagnostics (RED-4)", async () => {
    ...
    const secret = "opaque-sentry-token-" + "E".repeat(32);
    globalThis.fetch = (async () =>
      new Response(`request_id=req_sentry rejected credential ${secret}`, {
        status: 403,
      })) as typeof fetch;
    try {
      const client = makeClient({ token: secret, host: "https://sentry.example.test" });
      await assert.rejects(
        ...
        (err: unknown) => {
          ...
          assert.match(err.message, /request_id=req_sentry/);
