...
for cand in \
  ...
  "$HOME/.config/kit/internal-terms"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then TERMS_FILE="$cand"; break; fi
done

if [ -z "$TERMS_FILE" ]; then
  echo "no-internal-leaks: no term list found — set INTERNAL_TERMS_FILE or create ~/.config/kit/internal-terms (skipping)" >&2
  exit 0
fi
